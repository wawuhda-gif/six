#!/bin/bash
# Zivpn UDP Module installer - FIXED CONFIG
# Creator Zahid Islam | Extended Edition
...
apt-get install -y -q curl wget openssl iptables ufw python3 python3-pip sqlite3 net-tools jq git
...
echo -e "${YELLOW}${BOLD}[4/8] Tuning network...${NC}"
echo -e "${CYAN}${LINE}${NC}"
...
# ── CONFIG.JSON YANG BENAR - hanya auth.config, tidak ada duplikat ──
echo -e "${YELLOW}${BOLD}[6/8] Writing config.json...${NC}"
echo -e "${CYAN}${LINE}${NC}"
...
EOF
echo -e "${GREEN}✓ config.json OK (format benar)${NC}\n"
